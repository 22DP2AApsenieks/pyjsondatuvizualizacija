...
from tkinter import filedialog, messagebox #message = dialogs (alerts, confirmations) //fieldialog = priekš iperacijam ar failiem(open/save)
import os
import json
import sys
...
class JSONTimeStampSaglabatajs:
    def __init__(self, root):
        self.root = root   #saglabas informaciju loga
    ...
    def create_widgets(self):
        """Izveido logu lietotaja vajadzībām."""
        for i in range(1, 5):
    ...
    def browse_directory(self, dir_num):
        """Lai lietotājs varētu izvēlēties direktoriju."""
                    ...
                    for file in files:
                        if file.lower().endswith('.json'):
                            ...
                            try:
                                ...
                                copy_data = {   #viss ko gribam saglabāt timestamp faila
    ...
    def delete_timestamp_files(self):
        """Izdezēš visus failus, kas sākas ar timestamp"""
        if not any(self.directories.values()):
            messagebox.showerror("Kļūda", "Lūdzu, atlasiet vismaz vienu mapi!")
            return
                ...
                for file in files:
                    if file.startswith("timestamp_"):
                        ...
                        try:
                            ...
                            total_deleted += 1
                        except Exception as e: #negaiditiem error  //e ir mainigais, kura tiek sagalabats kludas
